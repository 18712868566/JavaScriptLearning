window.onload = function(){
	...
	for (var i = 0; i < aInputs.length; i++) {
		...
		if (aInputs[i].type == 'checkbox' && aInputs[i] != oBtnSelectAll) {
			aInputs[i].onclick = calcTotalPrive;
		}
	}
	...
	function interlaceColor() {
		...
		for (var i = 0; i < aRows.length; i++) {
			if (i % 2) {
				aRows[i].style.backgroundColor = '#fafafa';
			}else{
				aRows[i].style.backgroundColor = '';
			}
		}
	};
	// 删除
	...
	function sortTable(fnCmp) {
		...
		for (var i = 0; i < aRowsForSort.length; i++) {
