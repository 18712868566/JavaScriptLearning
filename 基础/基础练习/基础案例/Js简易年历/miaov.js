window.onload = function(){
	...
	var aList = oTab.getElementsByTagName('li');
	var arr = [
	"快过年了，大家可以商量着去哪玩吧～",
	...
	var oText = getByClass(oTab,'text');
	//console.log(oText[0]);
	...
	for (var i = 0; i < aList.length; i++) {
		...
		aList[i].onmouseover = function(){
			...
			oText[0].innerHTML = '<h2>'+ (this.index+1) +'月活动</h2><p>'+ arr[this.index] +'</p>'
		}
	}
...
function getByClass(parent,sClassName) {
	...
	for (var i = 0; i < elEm.length; i++) {
		if (elEm[i].className == sClassName) {
			sArr.push(elEm[i]);
		}
	}
	return sArr;
