...
$(".product-detail .pro-img").lazyload()
...
if($("#limit").attr("max")>1){e=$("#limit").attr("max")
}else{e=this.SetNumData.max
...
}var j=$(".content-address .sn-tab .city-list");
...
}var J=$(".detail-desc"),r=$(".detail-desc .cmp-nav"),x=$(".detail-desc .sn-tab-content>div");
...
function p(Q,L,M){var P,R;
if(M){P=M.hideFn;
R=M.hash
}var O=$(window).height();
Q.addClass("page-slideLeft");
var N=O-(P?P():n(Q));
L.css({height:O-(P?P():n(Q))});
...
var M=$(".detail-desc .sn-tab-nav li.current").index();
x.eq(M).css({height:"auto"}).siblings().css({height:0});
...
},A,k,getItempriceInfo],initCallback:[A,k,getItempriceInfo]})
};
